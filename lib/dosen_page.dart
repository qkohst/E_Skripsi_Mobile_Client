...
class Dosen extends StatefulWidget {
  @override
  _DosenState createState() => new _DosenState();
}

class _DosenState extends State<Dosen> {
  late List<DosenModel> listdosen;
  @override
  void initState() {
    super.initState();
  }

  Future<List<DosenModel>> _fetchData() async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      var _apiToken = prefs.getString('api_token');
      var jsonResponse = await http.get(
          Uri.parse(
              'http://192.168.43.71:8000/api/v1/adminprodi/dosen?api_key=4pyHNMVxTnNVpe571jMMXtE0a5X74PDYqkoLRds9lXlSqOW7GtqrBT29ptY5'),
          headers: {
      ...
      if (jsonResponse.statusCode == 200) {
        ...
        listdosen = jsonItems.map<DosenModel>((json) {
  ...
  Future<Null> _refresh() {
    return _fetchData().then((_listdosen) {
      setState(() => listdosen = _listdosen);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: RefreshIndicator(
          onRefresh: _refresh,
          child: FutureBuilder<List<DosenModel>>(
            future: _fetchData(),
