...
class Mahasiswa extends StatefulWidget {
  @override
  _MahasiswaState createState() => new _MahasiswaState();
}

class _MahasiswaState extends State<Mahasiswa> {
  late List<MahasiswaModel> listmahasiswa;
  @override
  void initState() {
    super.initState();
  }

  Future<List<MahasiswaModel>> _fetchData() async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      var _apiToken = prefs.getString('api_token');
      var jsonResponse = await http.get(
          Uri.parse(
              'http://192.168.43.71:8000/api/v1/adminprodi/mahasiswa?api_key=4pyHNMVxTnNVpe571jMMXtE0a5X74PDYqkoLRds9lXlSqOW7GtqrBT29ptY5'),
          headers: {
      ...
      if (jsonResponse.statusCode == 200) {
        ...
        listmahasiswa = jsonItems.map<MahasiswaModel>((json) {
  ...
  Future<Null> _refresh() {
    return _fetchData().then((_listmahasiswa) {
      setState(() => listmahasiswa = _listmahasiswa);
    });
  }
